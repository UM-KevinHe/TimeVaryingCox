...
{ int p=z.n_cols;
  arma::mat theta=arma::zeros<arma::mat>(p,knot);
  ...
  arma::rowvec test_constrast=arma::zeros<arma::rowvec>(p);
  arma::mat constrain=-diff(arma::diagmat(ones<arma::rowvec>(knot)));
  arma::mat GVG=result["GVG"];
...
List ddloglik_stratify(int knot,arma::colvec &facility,arma::mat &z,arma::colvec &delta,arma::mat &b_spline,arma::mat &theta,int number_facility=1){
  ...
  arma::mat L2=arma::zeros<arma::mat>(p*knot,knot);
  double partial_likelihood=0;
  arma::rowvec S0=arma::zeros<arma::rowvec>(n);
  arma::rowvec S1;
  arma::rowvec S2;
